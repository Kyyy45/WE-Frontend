...
export default function AdminLayout({
  ...
  // Guard: Redirect jika tidak login atau bukan admin
  useEffect(() => {
    // Kita beri sedikit delay agar state auth sempat ter-hydrate
    const checkAuth = setTimeout(() => {
      if (!isAuthenticated) {
        router.replace("/login");
      } else if (user?.role !== "admin") {
        router.replace("/dashboard/siswa"); // Redirect unauthorized user ke dashboard siswa
      }
    }, 100);
  ...
  // Loading state sederhana saat cek role
